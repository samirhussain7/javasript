...
async function showCountryDetails() {
  ...
  document.title = `${country.name} | Country Data`;
  document.querySelector('#favicon').href = country.flag;

  const borders = country.borders || [];
  const borderHTML = borders.length > 0 
    ? borders.map(b => `<span class="border-country">${b}</span>`).join(' ') 
  ...
  allInfo.innerHTML = `
        ...
        <div class="text-content">
            ...
            <div class="border"><b>Border Countries:</b> ${borderHTML}</div>
        </div>
    `;
}
showCountryDetails();
